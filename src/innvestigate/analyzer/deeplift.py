...
def _create_deeplift_rules(reference_mapping, approximate_gradient=True):
    def RescaleRule(Xs, Ys, As, reverse_state, local_references=None):
...
class DeepLIFT(base.ReverseAnalyzerBase):
    ...
    def _state_to_kwargs(cls, state):
        ...
        kwargs = super(DeepLIFT, cls)._state_to_kwargs(state)
...
class DeepLIFTWrapper(base.AnalyzerNetworkBase):
    ...
    def _state_to_kwargs(cls, state):
        ...
        kwargs = super(DeepLIFTWrapper, cls)._state_to_kwargs(state)
        kwargs.update(
            {
